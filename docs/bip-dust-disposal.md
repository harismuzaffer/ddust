...
2. The fee rate MUST be at least the minimum transaction relay fee rate (0.1 sat/vB at the time of writing).

#### Output

1. The transaction MUST have exactly one output.
2. The single output MUST be an OP_RETURN.
3. The OP_RETURN data MUST be the three-byte ASCII string "ash":  
...
1. Signed dust inputs MUST be collected from unconfirmed transactions found in nodes' mempools.
2. Inputs MUST NOT be received directly from other wallet users.
3. All inputs MUST be spent to the same single OP_RETURN `ash` output, see the "Output" section above.
...
- OP_RETURN outputs relay policy (Bitcoin Core 0.9.0+)
...
2. Spending multiple dust UTXOs for the same address produces a single "ash" OP_RETURN output regardless of script type.
3. Spending a single 2-of-2 P2SH multisig dust UTXO produces a single "ash" OP_RETURN output.
...
1. Adding a Bech32m dust input to an unconfirmed disposal transaction with a legacy dust input keeps the original single "ash" OP_RETURN output.
